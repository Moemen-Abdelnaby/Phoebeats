...
export function dbRecordPlayEvent(track: Track, secs: number): Promise<void> {
  statsWrites = statsWrites.then(() => writePlayEvent(track, secs));
  return statsWrites;
}
async function writePlayEvent(track: Track, secs: number): Promise<void> {
  try {
    if (!track.url) return;
    await invoke("db_record_play_event", {
      url: track.url,
      title: secs > 0 ? "" : track.title || "Unknown",
      artist: track.artist || "",
...
export async function dbGetListeningHistory(
  limit = 100,
...
export function dbClearListeningStats(): Promise<void> {
  statsWrites = statsWrites.then(clearListeningStats);
  return statsWrites;
}
async function clearListeningStats(): Promise<void> {
  try {
...
export async function dbSearchLibrary(
  query: string,
