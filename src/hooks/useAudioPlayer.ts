...
export function useAudioPlayer({
  ...
  const resumePositionRef = useRef(
    loadLS<{ url: string; seconds: number } | null>("pb_resumePosition", null),
  );
  ...
  const setProgressSeconds = publishPlaybackProgress;
  ...
  const [shuffle, setShuffle] = useState<boolean>(() =>
    loadLS("vg_shuffle", false),
  );
  const [repeatMode, setRepeatMode] = useState<RepeatMode>(() =>
    loadLS("vg_repeat", "off"),
  );
  const repeatModeRef = useRef<RepeatMode>(repeatMode);

  const playlistContextRef = useRef<{ tracks: Track[]; index: number } | null>(
    null,
  );
  ...
  useEffect(() => {
    const persist = () => {
      if (currentTrackRef.current && trackDurationRef.current > 0)
        saveLS("pb_resumePosition", {
          url: currentTrackRef.current.url,
          seconds: progressSecondsRef.current,
        });
    };
  ...
  const cycleRepeat = useCallback(() => {
    setRepeatMode((p) => {
      ...
      saveLS("vg_repeat", n);
      showToast(
        n === "off" ? "Repeat off" : n === "all" ? "Repeat all" : "Repeat one",
      );
      return n;
    });
  }, [showToast]);
  ...
  const handlePlayTrack = useCallback(
    async (track: Track, fromQueue = false) => {
      ...
      if (!fromQueue) {
        ...
        if (playlistContextRef.current) {
          const idx = playlistContextRef.current.tracks.findIndex(
            (t) => t.url === track.url,
          );
          if (idx >= 0)
      ...
      setQuickPicks((prev) =>
        [track, ...prev.filter((t) => t.url !== track.url)].slice(0, 20),
      );
      ...
      try {
        ...
        if (
          resume?.url === track.url &&
          Number.isFinite(resume.seconds) &&
          resume.seconds > 0
        ) {
          await invoke("seek_audio", { time: resume.seconds }).catch(() => {});
        }
    ...
    [
      ...
      setQuickPicks,
      setIsPlayingSync,
      setLoadingTrackUrlSync,
      showToast,
      setLyricsData,
    ],
  );

  const handlePlayLocalTrack = useCallback(
    async (
      local: LocalTrack,
      ...
      if (localList !== undefined) {
        ...
        const idx = localTracksListRef.current.findIndex(
          (t) => t.path === local.path,
        );
      ...
      if (!cover) {
        try {
          const coverB64 = await invoke<string | null>("get_audio_cover", {
            path: local.path,
          });
      ...
      setQuickPicks((prev) =>
        [synth, ...prev.filter((t) => t.url !== synth.url)].slice(0, 20),
      );
      ...
      try {
        ...
        await invoke("set_playback_speed", { speed: 1 });

        setTimeout(async () => {
          try {
            const s: { position: number; duration: number } =
  ...
  const toggleMute = useCallback(async () => {
    const targetVol =
      volume === 0 ? (previousVolume > 0 ? previousVolume : 80) : 0;
    if (volume > 0) setPreviousVolume(volume);
    setVolume(targetVol);
    try {
      await invoke("set_volume", { volume: targetVol });
    } catch {}
  ...
  useEffect(() => {
    if (!isPlaying || !currentTrack || !onListeningStep) return;
  ...
  }, [isPlaying, currentTrack?.url, onListeningStep]);
  ...
  const updateProgressFromEvent = useCallback((clientX: number) => {
    ...
    const total =
      ...
      parseDurationToSeconds(currentTrackRef.current.duration);
    const t = total * pct;
  ...
  const calculateProgressPercent = useCallback(() => {
    const dur =
      ...
      parseDurationToSeconds(currentTrack?.duration || "0:00");
    if (!dur) return 0;
    return Math.min(100, (progressSecondsRef.current / dur) * 100);
  }, [trackDurationSeconds, currentTrack?.duration]);
  ...
  const prefetchOnHover = useCallback((url: string) => {
    if (!url || url.startsWith("local://") || prefetchedSetRef.current.has(url))
      return;
