...
export function useLyrics(
  ...
  const [lyricsSource, setLyricsSourceState] = useState<string>(() =>
    loadLS("vg_lyricsSource", "lrclib"),
  );
  ...
  useEffect(() => {
    ...
    if (
      currentIdx !== lastScrolledLyricIdxRef.current ||
      !el.getAttribute("data-scrolled")
    ) {
  ...
  useEffect(() => {
    ...
    const onResize = () => {
      if (lyricsScrollContainerRef.current) {
        const active = lyricsScrollContainerRef.current.querySelector(
          '[data-active="true"]',
        ) as HTMLElement;
        if (active)
