...
export function usePlaylists(showToast?: (msg: string) => void) {
  const [playlists, setPlaylistsState] = useState<Playlist[]>(() =>
    loadLS("vg_playlists", [
      { id: "p1", name: "Liked Songs", description: "", tracks: [] },
    ]),
  );
  const [openPlaylistId, setOpenPlaylistId] = useState<string | null>(() =>
    loadLS("pb_lastPlaylist", null),
  );
  useEffect(() => {
    saveLS("pb_lastPlaylist", openPlaylistId);
  }, [openPlaylistId]);
  const [selectedPlaylistIds, setSelectedPlaylistIds] = useState<string[]>([]);
  const [isPlaylistMultiSelect, setIsPlaylistMultiSelect] =
    useState<boolean>(false);
  const [playlistDeleteModal, setPlaylistDeleteModal] = useState<{
    ids: string[];
    names: string[];
  } | null>(null);
  const [playlistSearchQ, setPlaylistSearchQ] = useState("");
  const [playlistViewMode, setPlaylistViewMode] = useState<"grid" | "list">(
    () => loadLS("vg_playlistViewMode", "grid"),
  );
  ...
  const savedPlaylists = useRef(new Map<string, Playlist>());
  useEffect(() => {
    saveLS("vg_playlists", playlists);
    playlists.forEach((p) => {
      if (savedPlaylists.current.get(p.id) !== p) void dbSavePlaylist(p);
    });
    savedPlaylists.current = new Map(playlists.map((p) => [p.id, p]));
  }, [playlists]);
  ...
  const setPlaylists = useCallback(
    (playlistsOrUpdater: Playlist[] | ((prev: Playlist[]) => Playlist[])) => {
      setPlaylistsState((prev) => {
        const next =
          typeof playlistsOrUpdater === "function"
            ? playlistsOrUpdater(prev)
            : playlistsOrUpdater;
        return next;
      });
    },
  ...
  const confirmDeletePlaylist = useCallback(() => {
    ...
    if (showToast)
      showToast(
        count === 1 ? "Playlist deleted" : `${count} playlists deleted`,
      );
  ...
  const confirmRenamePlaylist = useCallback(() => {
    ...
    setPlaylists((p) =>
      p.map((x) =>
        x.id === renamingPlaylist.id
          ? { ...x, name: renameVal.trim(), description: renameDescVal.trim() }
          : x,
      ),
    );
    ...
    (pid: string, url: string) => {
      setPlaylists((p) =>
        p.map((x) =>
          x.id !== pid
            ? x
            : { ...x, tracks: x.tracks.filter((t) => t.url !== url) },
        ),
      );
      ...
      inp.onchange = (e) => {
        ...
        if (f) {
          ...
          r.onload = (ev) => {
            ...
            if (d) {
              setPlaylists((p) =>
                p.map((x) => (x.id === pid ? { ...x, customCover: d } : x)),
              );
  ...
  const reorderPlaylistTracks = useCallback(
    (pid: string, fromIdx: number, toIdx: number) => {
      setPlaylists((p) =>
        p.map((x) => {
          if (x.id !== pid) return x;
          if (
            fromIdx < 0 ||
            fromIdx >= x.tracks.length ||
            toIdx < 0 ||
            toIdx >= x.tracks.length
          )
            return x;
          const next = [...x.tracks];
