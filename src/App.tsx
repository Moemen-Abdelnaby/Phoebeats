...
import { LibraryRepair } from "./components/LibraryRepair";
...
import { CompactPlayer as PlayerBar } from "./components/layout/CompactPlayer";
...
function normalizeNav(nav: string): NavView {
  if (nav === "home") return "playlists";
  return (
    [
      "home",
      "downloads",
...
export function App() {
  ...
  const [cacheEnabled, setCacheEnabledState] = useState<boolean>(() =>
    loadLS("vg_cacheEnabled", true),
  );
  ...
  const [loudnormEnabled, setLoudnormEnabledState] = useState<boolean>(() =>
    loadLS("vg_loudnorm", false),
  );
  const [skipSilence, setSkipSilenceState] = useState<boolean>(() =>
    loadLS("vg_skipSilence", false),
  );
  const [downloadQuality, setDownloadQuality] = useState<string>(() =>
    loadLS("vg_dlQuality", "High"),
  );
  const [downloadFormat, setDownloadFormatState] = useState<string>(() =>
    loadLS("vg_dlFormat", "mp3"),
  );
  const [embedThumbnail, setEmbedThumbnailState] = useState<boolean>(() =>
    loadLS("vg_embedThumb", true),
  );
  const [duplicateDetect, setDuplicateDetectState] = useState<boolean>(() =>
    loadLS("vg_dupDetect", true),
  );
  ...
  const [discordTimeDisplay, setDiscordTimeDisplay] = useState<
    "remaining" | "elapsed"
  >(() => {
    const v = loadLS<string>("vg_discordTimeDisplay", "remaining");
    return v === "elapsed" ? "elapsed" : "remaining";
  });
  const [discordCustomBtn, setDiscordCustomBtn] = useState<boolean>(() =>
    loadLS("vg_discordCustomBtn", false),
  );
  const [discordBtnLabel, setDiscordBtnLabel] = useState<string>(() =>
    loadLS("vg_discordBtnLabel", ""),
  );
  const [discordBtnUrl, setDiscordBtnUrl] = useState<string>(() =>
    loadLS("vg_discordBtnUrl", ""),
  );
  ...
  useEffect(() => {
    saveLS("vg_volume", volume);
  }, [volume]);
  useEffect(() => {
    saveLS("vg_dlQuality", downloadQuality);
  }, [downloadQuality]);
  useEffect(() => {
    saveLS("vg_dlFormat", downloadFormat);
  }, [downloadFormat]);
  useEffect(() => {
    saveLS("vg_embedThumb", embedThumbnail);
  }, [embedThumbnail]);
  useEffect(() => {
    saveLS("vg_dupDetect", duplicateDetect);
  }, [duplicateDetect]);
  useEffect(() => {
    saveLS("vg_dlPath", downloadPath);
  }, [downloadPath]);
  useEffect(() => {
    saveLS("vg_eq", eq);
  }, [eq]);
  useEffect(() => {
    saveLS("vg_trayEnabled", trayEnabled);
  }, [trayEnabled]);
  useEffect(() => {
    saveLS("vg_discordRpcEnabled", discordRpcEnabled);
  }, [discordRpcEnabled]);
  useEffect(() => {
    saveLS("vg_discordShowCover", discordShowCover);
  }, [discordShowCover]);
  useEffect(() => {
    saveLS("vg_discordTimeDisplay", discordTimeDisplay);
  }, [discordTimeDisplay]);
  useEffect(() => {
    saveLS("vg_discordCustomBtn", discordCustomBtn);
  }, [discordCustomBtn]);
  useEffect(() => {
    saveLS("vg_discordBtnLabel", discordBtnLabel);
  }, [discordBtnLabel]);
  useEffect(() => {
    saveLS("vg_discordBtnUrl", discordBtnUrl);
  }, [discordBtnUrl]);
  ...
  } = useLyrics(currentTrack, trackDurationSeconds);

  useEffect(() => {
    if (!isPlaying) flushListeningStats();
  }, [isPlaying, flushListeningStats]);
  ...
  const [metadataEditingTrack, setMetadataEditingTrack] =
  ...
  const getTrackCover = useCallback(
  ...
  const getPlaylistCover = usePlaylistArtwork(playlists);
  useEffect(() => {
    if (playlistContextRef.current || !currentTrack) return;
    const playlist =
      playlists.find((p) => p.id === openPlaylistId) ||
      playlists.find((p) => p.tracks.some((t) => t.url === currentTrack.url));
    const index =
      playlist?.tracks.findIndex((t) => t.url === currentTrack.url) ?? -1;
    if (playlist && index >= 0)
      playlistContextRef.current = { tracks: playlist.tracks, index };
  }, [playlists, openPlaylistId, currentTrack, playlistContextRef]);

  const [startupNav, setStartupNavState] = useState<string>(() => {
    const nav = loadLS<string>("vg_startupNav", "home");
    return nav === "last" ? nav : normalizeNav(nav);
  });
  const setStartupNav = useCallback((nav: string) => {
    const next = nav === "last" ? nav : normalizeNav(nav);
    setStartupNavState(next);
    saveLS("vg_startupNav", next);
  }, []);

  const [activeNav, setActiveNavState] = useState<NavView>(() => {
    const startup = loadLS<string>("vg_startupNav", "home");
    if (startup === "last")
      return normalizeNav(loadLS<string>("vg_lastNav", "home"));
    if (startup === "library" || startup === "playlists") return "playlists";
    if (["home", "downloads", "stats", "history", "settings"].includes(startup))
      return normalizeNav(startup);
    return "playlists";
  });
  ...
  const [navHistory, setNavHistory] = useState<NavView[]>(() => {
    ...
    const initial: NavView =
      startup === "last"
        ? normalizeNav(loadLS<string>("vg_lastNav", "home"))
        : startup === "library" || startup === "playlists"
          ? "playlists"
          : ["home", "downloads", "stats", "history", "settings"].includes(
                startup,
              )
            ? normalizeNav(startup)
            : "playlists";
    return [initial];
  });
  ...
  useEffect(() => {
    ...
    window.addEventListener("phoebeats-open-appearance", recover);
    return () =>
      window.removeEventListener("phoebeats-open-appearance", recover);
  }, []);
  ...
  const setActiveNav = useCallback((nav: NavView) => {
    ...
    setNavHistory((prev) =>
      prev[prev.length - 1] === nav ? prev : [...prev, nav],
    );
  }, []);

  useEffect(() => {
    if (activeNav === "home") setActiveNav("playlists");
  }, [activeNav, setActiveNav]);

  useEffect(() => {
    const open = () => setPanelOpen(true);
    window.addEventListener("phoebeats-open-appearance", open);
    return () => window.removeEventListener("phoebeats-open-appearance", open);
  }, []);
  ...
  const [droppedCsvBatch, setDroppedCsvBatch] = useState<{
    id: number;
    items: { name: string; getText: () => Promise<string> }[];
  } | null>(null);
  ...
  const [bgYtImport, setBgYtImport] = useState<{ progress: number } | null>(
    null,
  );
  const [pendingSpotifyImport, setPendingSpotifyImport] = useState<{
    tracks: Track[];
    matchedCount: number;
    failedCount: number;
  } | null>(null);
  const [showDuplicatesPlaylist, setShowDuplicatesPlaylist] =
    useState<Playlist | null>(null);
  const [bulkEditPlaylist, setBulkEditPlaylist] = useState<Playlist | null>(
    null,
  );

  const openCtx = useCallback(
    (e: React.MouseEvent, menu: Omit<CtxMenu, "x" | "y">) => {
      e.preventDefault();
      e.stopPropagation();
  ...
  useEffect(() => {
    ...
    listen("mpris_play_pause", () => mprisToggleRef.current()).then((fn) =>
      unlisteners.push(fn),
    );
    listen("mpris_next", () => mprisNextRef.current()).then((fn) =>
      unlisteners.push(fn),
    );
    listen("mpris_prev", () => mprisPrevRef.current()).then((fn) =>
      unlisteners.push(fn),
    );
    listen("tray_play_pause", () => mprisToggleRef.current()).then((fn) =>
      unlisteners.push(fn),
    );
    listen("tray_next", () => mprisNextRef.current()).then((fn) =>
      unlisteners.push(fn),
    );
    listen("tray_prev", () => mprisPrevRef.current()).then((fn) =>
      unlisteners.push(fn),
    );
  ...
  const discordStatus = useDiscordRpc({
    ...
    customButton: discordCustomBtn,
    buttonLabel: discordBtnLabel,
    buttonUrl: discordBtnUrl,
  });
  ...
  useEffect(() => {
    const h = (e: KeyboardEvent) => {
      ...
      if (e.ctrlKey || e.metaKey) {
        ...
        if (
          ...
          setUiScaleState((prev) => {
            ...
            showToast(
              `UI Scale: ${next > 0 ? `+${next}` : next} (${100 + next * 5}%)`,
            );
            return next;
          });
          return;
      ...
      if (
        !isInput &&
        ((e.altKey && (e.code === "ArrowLeft" || e.key === "ArrowLeft")) ||
          e.code === "BrowserBack")
      ) {
        e.preventDefault();
        navigateBack();
  ...
  const handleDownload = useCallback(
    async (track: Track) => {
      ...
      if (duplicateDetect) {
        try {
          const scanned: LocalTrack[] = await invoke("scan_downloads", {
            path: downloadPath,
          });
      ...
      if (flyoutAutoCloseTimerRef.current)
        clearTimeout(flyoutAutoCloseTimerRef.current);
  ...
  const handleDeleteLocalTrack = useCallback(
    async (t: LocalTrack) => {
      try {
  ...
  const handleSaveMetadata = useCallback(
    async (title: string, artist: string, album: string) => {
      ...
      try {
        ...
        const newPath: string = await invoke("rename_local_file", {
          oldPath: path,
          newTitle: title.trim(),
        });
        ...
        setQueue((prev) =>
          prev.map((t) =>
            t.url === metadataEditingTrack.url ? updateSynth(t) || t : t,
          ),
        );
        setPlayHistory((prev) =>
          prev.map((t) =>
        ...
        setLocalTracks((prev) =>
          prev.map((t) =>
            t.path === path ? { ...t, title, artist, path: newPath } : t,
          ),
        );
  ...
  const handleExportPlaylistM3u = useCallback(
  ...
  const handleExportLocalTracksM3u = useCallback(
    async (ts: LocalTrack[]) => {
      try {
    ...
    try {
      const cleanPlaylists = (playlists || []).map((p) => ({
        ...
        tracks: (p.tracks || []).filter(
          (t) => t && t.url && !t.url.startsWith("local://"),
        ),
      }));
      ...
      const cleanPlayHistory = (playHistory || []).filter(
        (t) => t && t.url && !t.url.startsWith("local://"),
      );
      const cleanQuickPicks = (quickPicks || []).filter(
        (t) => t && t.url && !t.url.startsWith("local://"),
      );
      ...
      const cleanListeningHistory = (listeningHistory || []).filter(
        (item) => item && item.url && !item.url.startsWith("local://"),
      );
      const cleanPlaybackHistory = (playbackHistory || []).filter(
        (item) =>
          item &&
          item.track &&
          item.track.url &&
          !item.track.url.startsWith("local://"),
      );
      ...
      if (resolvedBase) {
        const filePath =
          resolvedBase.replace(/[/\\]$/, "") + sep + "phoebeats_backup.json";
        ...
        a.href = url;
        a.download = "phoebeats_backup.json";
        a.click();
  ...
  const handleRestore = useCallback(() => {
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        const vol =
          typeof data.volume === "number" ? data.volume : data.vg_volume;
        ...
        const shuf =
          data.shuffle !== undefined ? data.shuffle : data.vg_shuffle;
        ...
        const loudnorm =
          data.loudnormEnabled !== undefined
            ? data.loudnormEnabled
            : data.vg_loudnormEnabled;
        if (loudnorm !== undefined)
          setLoudnormEnabledState(ls("vg_loudnormEnabled", Boolean(loudnorm)));

        const skipSil =
          data.skipSilence !== undefined
            ? data.skipSilence
            : data.vg_skipSilence;
        if (skipSil !== undefined)
          setSkipSilenceState(ls("vg_skipSilence", Boolean(skipSil)));

        if (data.cssThemes && Array.isArray(data.cssThemes.themes)) {
          const themes = data.cssThemes.themes.filter(
            (t: any) =>
              t &&
              typeof t.id === "string" &&
          ...
          // Restore CSS disabled so imported styles can be reviewed in Appearance.
          saveCssThemes({ themes, activeId: null });
        }
        ...
        if (customBg !== undefined)
          setCustomBgColor(ls("vg_customBgColor", customBg));

        const acc = data.accentColor || data.vg_accentColor;
        if (acc) setAccentColor(ls("vg_accentColor", acc));

        const scale =
          data.uiScale !== undefined ? data.uiScale : data.vg_uiScale;
        if (scale !== undefined) setUiScale(ls("vg_uiScale", Number(scale)));

        const perf =
          data.performanceMode !== undefined
            ? data.performanceMode
            : data.vg_performanceMode;
        if (perf !== undefined)
          setPerformanceMode(ls("vg_performanceMode", Boolean(perf)));

        const stNav = data.startupNav || data.vg_startupNav;
        if (stNav) setStartupNav(ls("vg_startupNav", stNav));

        const tray =
          data.trayEnabled !== undefined
            ? data.trayEnabled
            : data.vg_trayEnabled;
        if (tray !== undefined)
          setTrayEnabled(ls("vg_trayEnabled", Boolean(tray)));

        const dlQuality =
          data.downloadQuality || data.dlQuality || data.vg_dlQuality;
        if (dlQuality) setDownloadQuality(ls("vg_dlQuality", dlQuality));

        const dlFormat =
          data.downloadFormat || data.dlFormat || data.vg_dlFormat;
        ...
        const embedThumb =
          data.embedThumbnail !== undefined
            ? data.embedThumbnail
            : data.embedThumb !== undefined
              ? data.embedThumb
              : data.vg_embedThumb;
        if (embedThumb !== undefined)
          setEmbedThumbnailState(ls("vg_embedThumb", Boolean(embedThumb)));

        const dupDetect =
          data.duplicateDetect !== undefined
            ? data.duplicateDetect
            : data.dupDetect !== undefined
              ? data.dupDetect
              : data.vg_dupDetect;
        if (dupDetect !== undefined)
          setDuplicateDetectState(ls("vg_dupDetect", Boolean(dupDetect)));

        const cacheEn =
          data.cacheEnabled !== undefined
            ? data.cacheEnabled
            : data.vg_cacheEnabled;
        if (cacheEn !== undefined)
          setCacheEnabled(ls("vg_cacheEnabled", Boolean(cacheEn)));

        const cacheLim = data.cacheLimit || data.vg_cacheLimit;
        if (cacheLim) ls("vg_cacheLimit", cacheLim);

        const autoUpdates =
          data.autoCheckUpdates !== undefined
            ? data.autoCheckUpdates
            : data.vg_autoCheckUpdates;
        if (autoUpdates !== undefined)
          setAutoCheckUpdates(ls("vg_autoCheckUpdates", Boolean(autoUpdates)));

        const discordRpc =
          data.discordRpcEnabled !== undefined
            ? data.discordRpcEnabled
            : data.vg_discordRpcEnabled;
        if (discordRpc !== undefined)
          setDiscordRpcEnabled(ls("vg_discordRpcEnabled", Boolean(discordRpc)));

        const showCover =
          data.discordShowCover !== undefined
            ? data.discordShowCover
            : data.vg_discordShowCover;
        if (showCover !== undefined)
          setDiscordShowCover(ls("vg_discordShowCover", Boolean(showCover)));

        const timeDisplay =
          data.discordTimeDisplay || data.vg_discordTimeDisplay;
        if (timeDisplay)
          setDiscordTimeDisplay(
            ls(
              "vg_discordTimeDisplay",
              timeDisplay === "elapsed" ? "elapsed" : "remaining",
            ),
          );

        const customBtn =
          data.discordCustomBtn !== undefined
            ? data.discordCustomBtn
            : data.vg_discordCustomBtn;
        if (customBtn !== undefined)
          setDiscordCustomBtn(ls("vg_discordCustomBtn", Boolean(customBtn)));

        const btnLabel =
          data.discordBtnLabel !== undefined
            ? data.discordBtnLabel
            : data.vg_discordBtnLabel;
        ...
        const btnUrl =
          data.discordBtnUrl !== undefined
            ? data.discordBtnUrl
            : data.vg_discordBtnUrl;
        if (btnUrl !== undefined)
          setDiscordBtnUrl(ls("vg_discordBtnUrl", btnUrl));

        const proxyVal =
          data.networkProxy !== undefined
            ? data.networkProxy
            : data.vg_networkProxy;
        if (proxyVal !== undefined) ls("vg_networkProxy", proxyVal);

        const instVal =
          data.customInstance !== undefined
            ? data.customInstance
            : data.vg_customInstance;
        ...
        const lfmEn =
          data.lastfmEnabled !== undefined
            ? data.lastfmEnabled
            : data.vg_lastfmEnabled !== undefined
              ? data.vg_lastfmEnabled
              : data.vg_lfm_enabled;
    ...
    setTrayEnabled,
    setDownloadQuality,
    setDownloadFormatState,
    setDownloadPath,
    setBackupPath,
    setEmbedThumbnailState,
    setDuplicateDetectState,
    setCacheEnabled,
    setAutoCheckUpdates,
  ...
  const addLocalTracksToPlaylist = useCallback(
    (id: string, tracks: Track[]) => {
      setPlaylists((prev) =>
        prev.map((p) =>
          p.id === id
            ? {
                ...
                tracks: [
                  ...tracks.filter(
                    (t) => !p.tracks.some((existing) => existing.url === t.url),
                  ),
                  ...p.tracks,
                ],
              }
      ...
      showToast("Selected songs added to playlist");
    },
    [setPlaylists, showToast],
  );
  const openLocalTrackContext = useCallback(
    (e: React.MouseEvent, track: LocalTrack) =>
      openCtx(e, {
        type: "local",
        track: {
          id: 0,
  ...
  return (
    <div
      className="pb-app pb-wallpaper-layout"
      style={{
        display: "flex",
    ...
    >
      ...
      >
        ...
        >
          <div
            className="pb-panel-toolbar"
            style={activeNav === "settings" ? { display: "none" } : undefined}
          >
            <button
              onClick={() => {
                setActiveNav("playlists");
          ...
          <DownloadsFlyout
            ...
            onClearCompleted={() =>
              setActiveDownloads((prev) =>
                prev.filter((d) => d.status === "downloading"),
              )
            }
          ...
          {activeNav === "downloads" && (
            <DownloadsPanel
              ...
              onAddTracksToPlaylist={addLocalTracksToPlaylist}
          ...
          {activeNav === "settings" && (
            <LibraryRepair
              playlists={playlists}
              onRelink={(oldUrl, newUrl) => {
                const repair = (t: Track) =>
                  t.url === oldUrl ? { ...t, url: newUrl, cover: "" } : t;
                setPlaylists((prev) =>
                  prev.map((p) => ({ ...p, tracks: p.tracks.map(repair) })),
                );
                setQueue((prev) => prev.map(repair));
                if (currentTrack?.url === oldUrl)
                  setCurrentTrack(repair(currentTrack));
                if (playlistContextRef.current)
                  playlistContextRef.current = {
                    ...playlistContextRef.current,
                    tracks: playlistContextRef.current.tracks.map(repair),
                  };
              }}
            />
          )}
          {activeNav === "settings" && (
            <SettingsPanel
              ...
              onReset={() =>
                setConfirmModal({
                  message: "Reset all Phoebeats data? This cannot be undone.",
                  onConfirm: () => {
                    localStorage.clear();
      ...
      <PlayerBar
        ...
        onOpenPlaylists={() => {
          ...
          setActiveNav("playlists");
          const contextTracks = playlistContextRef.current?.tracks;
          if (contextTracks?.length) {
            const playingPlaylist = playlists.find(
              (p) =>
                p.tracks.length === contextTracks.length &&
                p.tracks.every(
                  (track, index) => track.url === contextTracks[index].url,
                ),
            );
            if (playingPlaylist) setOpenPlaylistId(playingPlaylist.id);
          }
          setPanelOpen(true);
        }}
        onOpenSettings={() => {
            ...
            >
              <div>
                ...
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && confirmCreatePlaylist()
                  }
                />
              </div>
              <div>
      ...
      {showShortcuts && (
        <div
          ...
          style={{
            ...
            justifyContent: "center",
            padding: "16px",
            background: "rgba(var(--v-bg0-rgb),0.88)",
          }}
            ...
            >
              {(
                ...
                action === null ? (
                  ...
                  >
                    <span style={{ fontSize: "12px", color: "var(--v-fg2)" }}>
                      {action}
                    </span>
                    <kbd
                      style={{
      ...
      {(showYtImportModal || bgYtImport) && (
        <YtImportModal
          ...
          onProgress={(progress) =>
            setBgYtImport(progress !== null ? { progress } : null)
          }
          onAbort={() => {
            setBgYtImport(null);
            setShowYtImportModal(false);
          ...
          onSavePlaylist={(name, desc, importedTracks) => {
            ...
            setPlaylists((prev) => [
              ...prev,
              {
                id,
                name,
                description: desc || "Imported from YouTube",
                tracks: importedTracks,
              },
            ]);
            ...
            setPlaylists((prev) => {
              ...
              return [
                ...prev,
                {
                  id,
                  name: cleanName,
                  description: desc || "Imported from Spotify",
                  tracks: importedTracks,
                },
              ];
            });
          ...
          onSave={(name, desc) => {
            ...
            setPlaylists((prev) => [
              ...prev,
              {
                ...
                name,
                description: desc || "Imported from Spotify",
                tracks: pendingSpotifyImport.tracks,
              },
            ]);
            showToast(
              `"${name}" saved (${pendingSpotifyImport.tracks.length} tracks)`,
            );
            setBgImport(null);
            setPendingSpotifyImport(null);
          }}
      ...
      {showDuplicatesPlaylist &&
        (() => {
          const duplicatesWithIndex = findDuplicateTracks(
            showDuplicatesPlaylist.tracks,
          );
          ...
          const handleRemoveAll = () => {
            const dupeIndices = new Set(
              duplicatesWithIndex.map((d) => d.originalIndex),
            );
            const cleanedTracks = showDuplicatesPlaylist.tracks.filter(
              (_, idx) => !dupeIndices.has(idx),
            );
            ...
            setShowDuplicatesPlaylist((prev) =>
              prev ? { ...prev, tracks: cleanedTracks } : null,
            );
            showToast(
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      Playlist:{" "}
                      <span style={{ color: "var(--v-fg2)", fontWeight: 600 }}>
                        {showDuplicatesPlaylist.name}
                      </span>
                    </p>
                  </div>
                  <button
                ...
                >
                  {duplicatesWithIndex.length === 0 ? (
                    ...
                    >
                      ...
                      >
                        All tracks in this playlist are unique.
                      </p>
                      <button
                        onClick={() => setShowDuplicatesPlaylist(null)}
                        style={{
                    ...
                    >
                      ...
                      >
                        ...
                        >
                          {duplicatesWithIndex.length} duplicate track
                          {duplicatesWithIndex.length > 1 ? "s" : ""} found
                        </span>
                        {duplicatesWithIndex.length > 1 && (
                          ...
                          >
                            ...
                            {duplicatesWithIndex.length})
                          </button>
                        )}
                      </div>

                      {duplicatesWithIndex.map(
                        ({ track: t, originalIndex }) => (
                          <div
                            key={originalIndex}
                            style={{
                              display: "flex",
          ...
          >
            ...
            >
              <div>
                ...
                >
                  {bulkEditPlaylist.tracks.length} tracks in{" "}
                  <span style={{ color: "var(--v-fg2)", fontWeight: 600 }}>
                    {bulkEditPlaylist.name}
                  </span>
                </p>
              </div>
              <button
      ...
      {bgImport && !showCsvImportModal && !pendingSpotifyImport && (
        <div
          ...
          style={{
            ...
            boxShadow:
              "0 12px 32px rgba(0,0,0,0.8), 0 0 0 1px rgba(255,255,255,0.06)",
            cursor: "pointer",
            userSelect: "none",
          ...
          >
            ...
            <button
              ...
              onMouseEnter={(e) => {
                ...
                e.currentTarget.style.background = "rgba(255,96,96,0.15)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.color = "var(--v-fg2)";
                e.currentTarget.style.background = "transparent";
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                {Math.round(bgYtImport.progress)}%
              </span>
            </div>
            <div
              style={{
            ...
            >
              <div
                style={{
                  ...
                  borderRadius: "1.5px",
                  background:
                    "linear-gradient(90deg, #ff1e27 0%, #ff4b55 100%)",
                  width: `${bgYtImport.progress}%`,
                }}
              />
            </div>
          </div>
      ...
      {toast && (
        <div
          style={{
            ...
            borderRadius: "9999px",
            boxShadow:
              "0 12px 32px rgba(0,0,0,0.85), 0 2px 8px rgba(0,0,0,0.6)",
            pointerEvents: "none",
