...
import { enrichLocalTracks } from "../utils/enrichLocalTracks";

export const LocalTrackCover = React.memo(
  ({
    path,
    cover,
    isActive,
    ...
    return (
      <FileMusic
        size={16}
        style={{ color: isActive ? "var(--v-fg2)" : "var(--v-fg3)" }}
      />
    );
  },
);
...
export const DownloadsPanel = React.memo(function DownloadsPanel({
  ...
  onAddTracksToPlaylist?: (playlistId: string, tracks: Track[]) => void;
  tracks: LocalTrack[];
  ...
  const [renameArtistInputFocused, setRenameArtistInputFocused] =
    useState(false);
  ...
  const [downloadsSortBy, setDownloadsSortBy] = useState<
    | "default"
    | "title_asc"
    | "title_desc"
    | "artist_asc"
  ...
  const { selectedUrls, isMultiSelectActive, toggleSelect, clearSelection } =
    useMultiSelect();
  ...
  const filtered = useMemo(() => {
    ...
    if (downloadsSortBy === "title_asc") {
      ...
      list.sort(
        (a, b) =>
          parseDurationToSeconds(a.duration || "0:00") -
          parseDurationToSeconds(b.duration || "0:00"),
      );
    ...
    return list;
  }, [tracks, searchQ, downloadsSortBy]);

  const scan = useCallback(async () => {
    const generation = ++scanGeneration.current;
    const cancelled = () => generation !== scanGeneration.current;
    ...
    setError(null);
    try {
      const raw: LocalTrack[] = await invoke("scan_downloads", {
        path: downloadPath,
      });
      if (cancelled()) return;
      const existingByPath = new Map(existingTracks.map((t) => [t.path, t]));
      const rawWithCovers = raw.map((newTrack) => {
        const existing = existingByPath.get(newTrack.path);
        if (existing && newTrack.duration != null) {
          return {
            ...newTrack,
      ...
      void invoke<DiskInfo>("get_disk_usage", {
        path: downloadPath,
      })
        .then((di) => {
          if (!cancelled()) setDiskInfo(di);
        })
        .catch(() => {});
      ...
      await enrichLocalTracks(
        ...
        async (t) => {
          const m: {
            ...
            duration: string;
            has_cover: boolean;
          } = await invoke("get_audio_metadata", { path: t.path });
          return {
            ...t,
            title: m.title || t.title,
            artist: cleanArtist(m.artist) || t.artist || undefined,
            duration: m.duration,
    ...
    >
      ...
      <div
        style={{
          ...
          marginBottom: "20px",
          background:
            "linear-gradient(135deg, rgba(255, 255, 255, 0.015) 0%, rgba(255, 255, 255, 0) 100%)",
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          >
            ...
            {diskInfo && (
              ...
              >
                {formatBytes(diskInfo.used_bytes)} used · {diskInfo.track_count}{" "}
                files
              </div>
            )}
          </div>
        ...
        >
          ...
          >
            <RefreshCw
              size={15}
              style={scanning ? { animation: "spin 0.8s linear infinite" } : {}}
            />
          </button>
        </div>
      </div>
      {!scanning && tracks.length > 0 && (
        ...
        >
          <div style={{ position: "relative", flex: 1, minWidth: "200px" }}>
            <Search
              ...
              style={{
                ...
                color: searchQ ? "var(--v-accent)" : "var(--v-fg3)",
                pointerEvents: "none",
            ...
            <input
              ref={searchRef}
              type="text"
              placeholder="Filter offline tracks…"
              value={searchQ}
          ...
          <ThemedSelect
            ...
            icon={
              <ArrowUpDown size={13} style={{ color: "var(--v-accent)" }} />
            }
            options={[
      ...
      {!scanning && tracks.length === 0 && !error && (
        ...
        >
          ...
          <div>
            ...
            >
              Download tracks from Home or change folder in Settings
            </div>
          </div>
        </div>
      )}
      ...
      {!scanning && tracks.length > 0 && (
        <>
          <div className="v-section-head">
            ...
            {!searchQ && !enriching && (
              <span style={{ fontSize: "10px", color: "var(--v-fg3)" }}>
                drag to reorder
              </span>
            )}
          </div>
          ...
          {filtered.length === 0 && searchQ && (
            ...
            >
              ...
              <p style={{ fontSize: "12px", color: "var(--v-fg3)" }}>
                No tracks match "{searchQ}"
              </p>
            </div>
          )}
          ...
          <div>
                  ...
                  >
                    {isDragOver && (
                      <div
                        style={{
                          ...
                          right: 0,
                          height: "1.5px",
                          background: "rgba(226,221,217,0.5)",
                          borderRadius: "1px",
                          zIndex: 10,
                          pointerEvents: "none",
                        }}
                    ...
                    >
                      {isSelected ? (
                        <CheckSquare
                          size={13}
                          style={{ color: "var(--v-accent)", margin: "0 auto" }}
                        />
                      ) : isMultiSelectActive ? (
                        <Square
                          size={13}
                          style={{ color: "var(--v-fg3)", margin: "0 auto" }}
                        />
                        ...
                        <svg
                          ...
                          style={{
                            animation:
                              "spin 0.9s cubic-bezier(0.4, 0, 0.2, 1) infinite",
                            margin: "0 auto",
                            display: "block",
                          }}
                        >
                    ...
                    <div
                      style={{
                        ...
                        flexShrink: 0,
                        background: isActive
                          ? "rgba(226,221,217,0.06)"
                          : "var(--v-bdr2)",
                        border: `1px solid ${isActive ? "rgba(226,221,217,0.1)" : "rgba(255,255,255,0.05)"}`,
                        overflow: "hidden",
                      }}
                    >
                      <LocalTrackCover
                    ...
                    <div className="v-track__info">
                      ...
                      <div className="v-track__artist">
                        {track.artist || track.extension.toUpperCase()} ·{" "}
                    ...
                    <div className="v-track__actions">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          ...
                          setRenameArtistVal(track.artist || "");
                        }}
                      >
                        <Pencil size={12} />
                      </button>
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          onOpenInFileManager(track.path);
                        }}
                      >
                        <FolderOpen size={12} />
                      </button>
                      <button
                        ...
                        onMouseEnter={(e) =>
                          (e.currentTarget.style.color = "#b05555")
                        }
                        onMouseLeave={(e) =>
                          (e.currentTarget.style.color = "var(--v-fg3)")
                        }
            ...
            {isMultiSelectActive && (
              <BatchActionBar
                ...
                onPlaySelected={() => {
                  const selected = filtered.filter((t) =>
                    selectedUrls.has(`local://${t.path}`),
                  );
            ...
            >
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  >
                    Change the title and artist of this local track.
                  </p>
                </div>
                <button
                  onClick={() => setRenaming(null)}
                  style={{
                    ...
                    borderRadius: "50%",
                    border: "none",
                    background: renameCloseHovered
                      ? "rgba(255, 255, 255, 0.08)"
                      : "transparent",
                    color: renameCloseHovered ? "var(--v-fg)" : "#8a817c",
                    cursor: "pointer",
              ...
              >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={renameArtistVal}
                    onChange={(e) => setRenameArtistVal(e.target.value)}
                    ...
                    style={{
                      ...
                      boxShadow: renameArtistInputFocused
                        ? "0 0 0 2px rgba(226, 221, 217, 0.15)"
                        : "none",
                      color: "var(--v-fg)",
                      borderRadius: "8px",
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    color: renameCancelHovered ? "var(--v-fg)" : "#8a817c",
                    background: renameCancelHovered
                ...
                <button
                  ...
                  style={{
                    ...
                    fontSize: "12.5px",
                    transition: "all 0.2s ease",
                    boxShadow: renameVal.trim()
                      ? "0 4px 12px rgba(226, 221, 217, 0.2)"
                      : "none",
                  }}
