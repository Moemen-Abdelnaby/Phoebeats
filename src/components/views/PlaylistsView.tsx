...
interface PlaylistsViewProps {
  ...
  setPlaylistDeleteModal?: (
    modal: { ids: string[]; names: string[] } | null,
  ) => void;
...
export const PlaylistsView: React.FC<PlaylistsViewProps> = React.memo(
  ({
    ...
    const [internalHoveredTrackUrl, setInternalHoveredTrackUrl] = useState<
      string | null
    >(null);
    const [playlistSortBy, setPlaylistSortBy] = useState<
      ...
      | "duration_desc"
    >("default");
    const hoveredTrackUrl =
      customHoveredTrackUrl !== undefined
        ? customHoveredTrackUrl
        : internalHoveredTrackUrl;
    const setHoveredTrackUrl =
      customSetHoveredTrackUrl || setInternalHoveredTrackUrl;
    ...
    const removeFromPlaylist =
      customRemoveFromPlaylist ||
      ((pid: string, url: string) => {
        setPlaylists((prev) =>
          prev.map((p) => {
    ...
    const [dragOverPlaylistIdx, setDragOverPlaylistIdx] = useState<
      number | null
    >(null);

    const dragPlaylistCardIdx = useRef<number | null>(null);
    const dragOverPlaylistCardIdxRef = useRef<number | null>(null);
    const [dragOverPlaylistCardIdx, setDragOverPlaylistCardIdx] = useState<
      number | null
    >(null);
    const [dragPlaylistCardIdxState, setDragPlaylistCardIdxState] = useState<
      number | null
    >(null);
          ...
          >
            ...
            >
              <div
                style={{
                  ...
                  background:
                    openPlaylist.id === "p1"
                      ? "linear-gradient(135deg,rgba(140,30,30,0.4) 0%,rgba(140,30,30,0.1) 100%)"
                      : "var(--v-bg3)",
                ...
                onClick={() =>
                  openPlaylist.id !== "p1" && handleCoverUpload(openPlaylist.id)
                }
                onMouseEnter={(e) => {
              ...
              >
                ...
                >
                  {openPlaylist.id === "p1" ? (
                    <Heart
                      size={56}
                      style={{ color: "#e05555", fill: "rgba(224,85,85,0.25)" }}
                    />
                  ) : (
                    <ListMusic size={56} style={{ color: "var(--v-fg3)" }} />
                  )}
                </div>
                {getPlaylistCover(openPlaylist) && (
                  <img
                    src={getPlaylistCover(openPlaylist)!}
                    style={{
                      position: "absolute",
                      inset: 0,
                      width: "100%",
              ...
              <div style={{ flex: 1, minWidth: 0, paddingBottom: "4px" }}>
                <span
                  style={{
                    ...
                    color:
                      openPlaylist.id === "p1" ? "#ff5e5e" : "var(--v-fg2)",
                    display: "block",
                    marginBottom: "6px",
                  }}
                >
                ...
                >
                  <span style={{ fontWeight: 600, color: "var(--v-fg2)" }}>
                    {openPlaylist.tracks.length}{" "}
                    {openPlaylist.tracks.length === 1 ? "song" : "songs"}
                  </span>
                </div>
                <div
                  style={{
                ...
                >
                  <button
                    ...
                    onClick={() => playAll(openPlaylist.tracks)}
                    disabled={!openPlaylist.tracks.length}
                    style={{
                      display: "flex",
                      alignItems: "center",
                      gap: "8px",
                  ...
                  {(() => {
                    ...
                    if (dupes > 0) {
                      return (
                        <button
                          onClick={() => {
                            const dupeIndices = new Set(
                              duplicates.map((d) => d.originalIndex),
                            );
                            const cleanedTracks = openPlaylist.tracks.filter(
                            ...
                            showToast(
                              `Removed ${dupes} duplicate track${dupes > 1 ? "s" : ""}`,
                            );
                          }}
                        ...
                        >
                          <Layers size={14} /> Remove {dupes} Duplicate
                          {dupes > 1 ? "s" : ""}
                        </button>
                      );
                    }
                  ...
                  {openPlaylist.id !== "p1" && (
                    <button
                      onClick={() => {
                        setRenamingPlaylist(openPlaylist);
                        setRenameVal(openPlaylist.name);
                        setRenameDescVal(openPlaylist.description);
                      }}
                      style={{
                        display: "flex",
                        alignItems: "center",
            ...
            {openPlaylist.tracks.length === 0 ? (
              ...
              >
                ...
                <p style={{ fontSize: "13px", color: "var(--v-fg2)" }}>
                  No tracks yet.
                </p>
              </div>
            ) : (
              <>
                ...
                >
                  ...
                  >
                    ...
                    />
                    <div style={{ flex: 1, minWidth: 0, paddingLeft: "14px" }}>
                      Title
                    </div>
                    <div
                      style={{
                  ...
                  {filteredTracks.length === 0 ? (
                    ...
                    >
                      ...
                      <p style={{ fontSize: "13px", color: "var(--v-fg2)" }}>
                        No results for "{playlistSearchQ}"
                      </p>
                    </div>
                  ) : (
                    <VirtualTrackList
                          ...
                          >
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <TrackRow
                                ...
                                isActive={
                                  currentTrack?.url === enrichedTrack.url
                                }
                                isHovered={
                                  hoveredTrackUrl === enrichedTrack.url
                                }
                                isLoadingTrack={
                                  (loadingTrackUrl === enrichedTrack.url ||
                                    (currentTrack?.url === enrichedTrack.url &&
                                      isLoadingTrack)) &&
                                  !isPlaying
                                }
                                ...
                                isDownloading={
                                  downloadingTracks[enrichedTrack.url] ?? 0
                                }
                                isSelected={selectedTrackUrls.has(
                                  enrichedTrack.url,
                                )}
                                isMultiSelectActive={isTrackMultiSelectActive}
                                onPlay={() =>
                                ...
                                onHoverEnter={() => {
                                  setHoveredTrackUrl(enrichedTrack.url);
                                  prefetchOnHover(enrichedTrack.url);
                                }}
                ...
                {isTrackMultiSelectActive && (
                  <BatchActionBar
                    selectedTracks={openPlaylist.tracks.filter((t) =>
                      selectedTrackUrls.has(t.url),
                    )}
                    playlists={playlists}
                    isPlaylistView={true}
                    onClearSelection={clearTrackSelection}
                    onPlaySelected={() => {
                      const selected = openPlaylist.tracks.filter((t) =>
                        selectedTrackUrls.has(t.url),
                      );
                    ...
                    onQueueSelected={() => {
                      ...
                      if (selected.length > 0 && addToQueue) {
                        addToQueue(selected);
                        showToast(
                          `Added ${selected.length} track${selected.length > 1 ? "s" : ""} to queue`,
                        );
                      }
                      clearTrackSelection();
                    }}
                      ...
                      setPlaylists((prev) =>
                        prev.map((p) => {
                          ...
                          const existingUrls = new Set(
                            p.tracks.map((t) => t.url),
                          );
                          const newTracks = selected.filter(
                            (t) => !existingUrls.has(t.url),
                          );
                          return { ...p, tracks: [...newTracks, ...p.tracks] };
                        }),
                      );
                        ...
                        prev.map((p) => {
                          ...
                          return {
                            ...
                            tracks: p.tracks.filter(
                              (t) => !selected.has(t.url),
                            ),
                          };
                        }),
                      );
                      showToast(
                        `Removed ${selected.size} track${selected.size > 1 ? "s" : ""} from playlist`,
                      );
                      clearTrackSelection();
                    }}
                    onDownloadSelected={() => {
          ...
          >
            <div className="v-library-container">
                  ...
                  >
                    {isPlaylistMultiSelect ? (
                      <>
                        <button
                          onClick={() => {
                            const deletable = playlists
                              .filter((p) => p.id !== "p1")
                              .map((p) => p.id);
                            if (
                              selectedPlaylistIds.length === deletable.length
                            ) {
                              setSelectedPlaylistIds([]);
                            } else {
                        ...
                        >
                          ...
                          playlists.filter((p) => p.id !== "p1").length
                            ? "Deselect All"
                            : "Select All"}
                        </button>
                        ...
                        >
                          <Trash2 size={13} />
                          <span>
                            Delete Selected ({selectedPlaylistIds.length})
                          </span>
                        </button>
                      ...
                      return (
                        <div
                          ...
                          style={{
                            ...
                            position: "relative",
                            border:
                              isPlaylistMultiSelect && isSelected
                          ...
                          onContextMenu={(e) =>
                            openCtx(e, { type: "playlist", playlist: pl })
                          }
                        >
                          {isDragTarget && (
                            <div
                              style={{
                                ...
                                pointerEvents: "none",
                                left:
                                  plIdx < (dragPlaylistCardIdx.current ?? 0)
                                    ? "-12px"
                                    : "auto",
                                right:
                                  plIdx > (dragPlaylistCardIdx.current ?? 0)
                          ...
                          <div
                            ...
                            style={{
                              opacity:
                                dragPlaylistCardIdxState === plIdx ? 0.45 : 1,
                              transform:
                                dragPlaylistCardIdxState === plIdx
                                  ? "scale(0.94)"
                                  : "none",
              ...
              <div className="v-library-sidebar">
                ...
                <div
                  className="v-library-sidebar-card"
                  style={{
                    display: "flex",
                    flexDirection: "column",
                    gap: "12px",
                        ...
                        .map((track: Track, i: number) => (
                          <div
                            ...
                            onClick={() =>
                              handlePlayInContext(
                                track,
                                playHistory.slice(0, 4),
                              )
                            }
                            className="v-library-recent-row"
                          >
                            <div
                              className="v-library-recent-art"
                              style={{
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div className="v-library-recent-title">
                                {track.title}
                              </div>
                              <div className="v-library-recent-artist">
