...
import { LocalPlaylistArtwork } from "./LocalPlaylistArtwork";
  ...
  ({
    ...
    <div
      ...
      onClick={(e) => {
        if (
          ...
          e.ctrlKey ||
          e.metaKey ||
          (isMultiSelectActive && onSelectToggle)
        ) {
          e.preventDefault();
          onSelectToggle?.(e);
        } else {
    ...
    >
      ...
      >
        <Music
          size={16}
          style={{ position: "absolute", color: "rgba(255,255,255,0.25)" }}
        />
        {track.url.startsWith("local://") ? (
          <LocalPlaylistArtwork key={track.url} track={track} />
        ) : (
          track.cover && (
            <img
              src={track.cover}
              ...
              style={{
                ...
                transform:
                  ...
                  (track.cover.includes("ytimg.com") ||
                    track.cover.includes("googleusercontent.com"))
                    ? "scale(1.35)"
                    : "none",
              }}
              onError={(e) => {
      ...
      <div className="v-track__actions">
        ...
        <button
          className="v-track__btn"
          title={
            isDownloading > 0 && isDownloading < 100
              ? `Downloading ${Math.round(isDownloading)}% (Click to cancel)`
              : "Download"
          }
          onClick={(e) => {
            e.stopPropagation();
...
export const TrackRowSkeleton = ({ index }: { index: number }) => (
  <div
    style={{
      ...
      gap: "12px",
      padding: "8px 12px",
      animation: `fadeUpSm 0.18s cubic-bezier(0.2,0,0,1) ${index * 40}ms both`,
    }}
  >
    <div
      style={{
