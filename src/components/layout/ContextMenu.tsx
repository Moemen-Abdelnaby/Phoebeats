  ...
  ({
    ...
    const openInYouTube = (trackOrUrl: Track | string) => {
      ...
      if (
        ...
        const ytId =
          trackOrUrl.url?.match(/[?&]v=([^&]+)/)?.[1] ||
          trackOrUrl.url?.split("youtu.be/")?.[1]?.split("?")?.[0];
        if (ytId) {
    ...
    const addTrackToPlaylist = (pid: string, track: Track) => {
      ...
      if (!target) return;
      const alreadyAdded = target.tracks.some((t) => t.url === track.url);
      setPlaylists((prev) =>
        prev.map((p) => {
          if (p.id === pid) {
    ...
    return (
      <>
        {ctxMenu &&
          (() => {
            const { track, playlist, localTracksList, localTrackIndex } =
              ctxMenu;
            ...
            if (ctxMenu.type === "local" && track) {
              ...
              return (
                <div
                  className="v-ctx custom-scrollbar"
                  style={{
                    position: "fixed",
                    zIndex: 9999,
                    width: `${menuWidth}px`,
                ...
                >
                  ...
                  <button
                    ...
                    className="v-ctx__item"
                  >
                    <PlaySquare size={14} /> Play Next
                  </button>
                  <button
                    onClick={() => {
                      if (addToQueue) addToQueue(track);
                      else {
                        setQueue((p) => [
                          ...p.filter((t) => t.url !== track.url),
                          track,
                  ...
                  <button
                    onClick={() => {
                      handleOpenInFileManager(localTrack.path);
                      setCtxMenu(null);
                    }}
                    className="v-ctx__item"
                  >
                  ...
                  <button
                    onClick={() => {
                      handleDeleteLocalTrack(localTrack);
                      setCtxMenu(null);
                    }}
                    className="v-ctx__item v-ctx__item--danger"
                  >
              ...
              return (
                ...
                >
                  ...
                  {isLocal ? (
                    <button
                      onClick={() => {
                        handlePlayLocalTrack(localTrack);
                        setCtxMenu(null);
                      }}
                      className="v-ctx__item"
                    >
                  ...
                  {isLocal && (
                    <>
                      ...
                      <button
                        onClick={() => {
                          handleOpenInFileManager(localFilePath);
                          setCtxMenu(null);
                        }}
                        className="v-ctx__item"
                      >
                  ...
                  {isLocal ? (
                    ...
                    <>
                      ...
                      <button
                        onClick={() => {
                          copyToClipboard(getTrackShareUrl(track));
                          setCtxMenu(null);
                        }}
                        className="v-ctx__item"
                      >
            ...
            if (
              (ctxMenu.type === "playlist" ||
                ctxMenu.type === "sidebar-playlist") &&
              playlist
            ) {
              const menuWidth = 200;
        ...
        {addToPlaylistTrack && (
          ...
          >
            ...
            >
              ...
              >
                {playlists.map((p) => {
                  const alreadyIn = p.tracks.some(
                    (t) => t.url === addToPlaylistTrack.url,
                  );
                  return (
                    <button
                      key={p.id}
                      onClick={() =>
                        !alreadyIn &&
                        addTrackToPlaylist(p.id, addToPlaylistTrack)
                      }
                      disabled={alreadyIn}
                      className="v-ctx__item"
                      style={{
                    ...
                    >
                      ...
                      {alreadyIn ? (
                        ...
                        <span
                          style={{ fontSize: "10px", color: "var(--v-fg3)" }}
                        >
                          {p.tracks.length}
                        </span>
                      )}
        ...
        {infoModalTrack &&
          (() => {
            const ytId =
              infoModalTrack.url?.match(/[?&]v=([^&]+)/)?.[1] ||
              infoModalTrack.url?.split("youtu.be/")?.[1]?.split("?")?.[0] ||
              "";
            const ytUrl = ytId
            ...
            const trackAudioInfo =
              infoModalTrack.url === currentTrack?.url ? audioInfo : null;
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    <div
                      style={{
                        ...
                        inset: 0,
                        background:
                          "linear-gradient(to bottom, transparent 0%, var(--v-bg2) 100%)",
                      }}
                    />
                    <button
                      onClick={() => setInfoModalTrack(null)}
                    ...
                    >
                      <div
                        style={{
                          ...
                          overflow: "hidden",
                          boxShadow:
                            "0 12px 32px rgba(0,0,0,0.7), 0 0 0 1px var(--v-bdr2)",
                          background: getTrackGradient(
                            infoModalTrack.title,
                            infoModalTrack.artist,
                          ),
                  ...
                  >
                    {[
                      infoModalTrack.duration &&
                        infoModalTrack.duration !== "0:00" && {
                          icon: <Clock size={9} />,
                          label: infoModalTrack.duration,
                        },
                      isYt && { icon: <Youtube size={9} />, label: "YouTube" },
                      trackAudioInfo?.codec &&
                        trackAudioInfo.codec !== "unknown" && {
                          icon: <BarChart2 size={9} />,
                          label: `${trackAudioInfo.codec.toUpperCase()}${trackAudioInfo.bitrate > 0 ? ` · ${Math.round(trackAudioInfo.bitrate / 1000)}k` : ""}`,
                        },
                      trackAudioInfo &&
                        trackAudioInfo.samplerate &&
                        trackAudioInfo.samplerate > 0 && {
                          icon: <Gauge size={9} />,
                          label: `${(trackAudioInfo.samplerate / 1000).toFixed(1)}kHz`,
                        },
                      trackAudioInfo?.channels && {
                        icon: <AlignLeft size={9} />,
                        label: trackAudioInfo.channels,
                      },
                      trackAudioInfo?.format && {
                        icon: <FileCode2 size={9} />,
                        label: trackAudioInfo.format,
                      },
                    ]
                      .filter(Boolean)
                      .map((item: any, i) => (
                    ...
                    {[
                      {
                        icon: Music,
                        label: "Title",
                        value: infoModalTrack.title,
                      },
                      {
                        icon: FileBadge2,
                        label: "Artist",
                        value: infoModalTrack.artist,
                      },
                      ...(ytId
                        ? [{ icon: Hash, label: "Video ID", value: ytId }]
                        : []),
                      ...
                      >
                        ...
                        <Copy
                          size={11}
                          style={{ color: "var(--v-fg3)", flexShrink: 0 }}
                        />
                      </div>
                    ))}
                  </div>
