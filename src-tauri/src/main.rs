    ...
    SOCKET_PATH.get_or_init(|| {
        ...
        #[cfg(unix)]
        {
            if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
                format!("{}/phoebeats-mpv-{}.sock", runtime_dir.trim_end_matches('/'), pid)
            } else {
                let tmp = std::env::temp_dir();
                format!("{}/phoebeats-mpv-{}.sock", tmp.to_string_lossy().trim_end_matches('/'), pid)
            }
        }
        #[cfg(windows)]
        {
            format!(r"\\.\pipe\phoebeats-mpv-{}", pid)
        }
    })
}
...
fn init_bin_paths() {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        paths.push(format!("{}\\Programs\\phoebeats-deps\\mpv",    local_app_data));
        paths.push(format!("{}\\Programs\\phoebeats-deps\\ffmpeg",  local_app_data));
        paths.push(format!("{}\\Programs\\phoebeats-deps",          local_app_data));
    ...
    eprintln!("[phoebeats] mpv     -> {}", mpv);
    eprintln!("[phoebeats] yt-dlp  -> {}", ytdlp);
    eprintln!("[phoebeats] ffprobe -> {}", ffprobe);
    eprintln!("[phoebeats] ffmpeg  -> {}", ffmpeg);
...
fn init_log_path(app: &tauri::AppHandle) {
    if let Ok(dir) = app.path().app_log_dir().or_else(|_| app.path().app_data_dir()) {
        ...
        let _ = LOG_PATH.set(dir.join("phoebeats_debug.log"));
    }
}
...
async fn download_song(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        // Drain stderr concurrently so downloader errors cannot fill the pipe.
        ...
        let error_reader = std::thread::spawn(move || {
            ...
            if let Some(err) = stderr {
                for line in BufReader::new(err).lines().map_while(Result::ok) {
                    ...
                    if recent.len() > 12 { recent.pop_front(); }
                }
            }
        ...
        let error_details = error_reader.join().unwrap_or_default();
        ...
        if status.success() {
            ...
            Err(if error_details.trim().is_empty() { "Download failed".to_string() } else { error_details })
        }
    })
    .await
    .map_err(|e| e.to_string())?
...
async fn cancel_download(app_handle: tauri::AppHandle, url: String) -> Result<(), String> {
    ...
    if let Some(dl) = dl_opt {
        ...
        // Retain partial files for retry. Never sweep unrelated files from the songs folder.
            ...
            if p.is_file() {
                if let Some(ext) = p.extension().and_then(|e| e.to_str()) {
                    if extensions.contains(&ext.to_lowercase().as_str()) {
                        if let Some(meta) = metadata::cached_track_metadata(&p) {
...
async fn find_missing_files(paths: Vec<String>) -> Result<Vec<String>, String> {
    if paths.len() > 100000 { return Err("Too many file paths".into()); }
    tokio::task::spawn_blocking(move || paths.into_iter().filter(|path| !std::path::Path::new(&expand_tilde(path)).is_file()).collect()).await.map_err(|e| e.to_string())
}
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            if let Err(e) = run_mpris_server(app_handle, rx).await {
                eprintln!("[phoebeats] MPRIS server error: {}", e);
            }
        });
    });
}
...
async fn run_mpris_server(
    ...
    impl MediaPlayer2 {
        ...
        #[dbus_interface(property)]
        fn identity(&self) -> &str { "Phoebeats" }
        #[dbus_interface(property)]
        fn desktop_entry(&self) -> &str { "phoebeats" }
        #[dbus_interface(property)]
    ...
    impl Player {
        ...
        fn metadata(&self) -> HashMap<String, OwnedValue> {
            ...
            map.insert("mpris:trackid".into(),
                OwnedValue::try_from(ZValue::new(ObjectPath::try_from("/org/phoebeats/track/1").unwrap())).unwrap());
    ...
    let conn = ConnectionBuilder::session()?
        .name("org.mpris.MediaPlayer2.phoebeats")?
    ...
    tokio::task::spawn_blocking(move || -> Result<(), String> {
        ...
        if let Some(ref mut client) = *client_lock {
            ...
            if let Err(error) = client.set_activity(act) {
                let _ = client.close();
                *client_lock = None;
                return Err(format!("Discord activity update failed: {error}"));
            }
        }
        Ok(())
