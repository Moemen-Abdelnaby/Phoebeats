...
fn get_db_path(app: &tauri::AppHandle) -> PathBuf {
    if let Ok(app_dir) = app.path().app_data_dir() {
        ...
        return app_dir.join("phoebeats.db");
    }
    
    // Fallback: standard config directory
    if let Some(user_dir) = dirs_fallback() {
        let dir = user_dir.join(".config").join("phoebeats");
        let _ = std::fs::create_dir_all(&dir);
        return dir.join("phoebeats.db");
    }

    PathBuf::from("phoebeats.db")
}
...
pub fn init_db(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    conn.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS local_metadata_cache (
...
pub fn load_local_metadata(path: &str, fingerprint: &str) -> Option<String> {
    with_db(|conn| conn.query_row("SELECT metadata_json FROM local_metadata_cache WHERE path=?1 AND fingerprint=?2", params![path, fingerprint], |row| row.get(0))).ok()
}

pub fn save_local_metadata(path: &str, fingerprint: &str, json: &str) {
    let _ = with_db(|conn| conn.execute("INSERT OR REPLACE INTO local_metadata_cache(path,fingerprint,metadata_json) VALUES(?1,?2,?3)", params![path, fingerprint, json]));
}
...
mod persistent_cache_tests {
    use super::*;
    #[test]
    fn metadata_survives_reopening_and_rejects_changed_files() {
        let path = std::env::temp_dir().join(format!("phoebeats-cache-test-{}-{}.db", std::process::id(), std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_nanos()));
        let conn = Connection::open(&path).unwrap();
        conn.execute_batch("CREATE TABLE local_metadata_cache(path TEXT PRIMARY KEY,fingerprint TEXT NOT NULL,metadata_json TEXT NOT NULL)").unwrap();
        assert!(DB_CONN.set(Mutex::new(conn)).is_ok());
        save_local_metadata("song.mp3", "size-and-mtime", "{\"title\":\"Song\"}");
        *DB_CONN.get().unwrap().lock().unwrap() = Connection::open(&path).unwrap();
        assert_eq!(load_local_metadata("song.mp3", "size-and-mtime").as_deref(), Some("{\"title\":\"Song\"}"));
        assert!(load_local_metadata("song.mp3", "changed").is_none());
        DB_CONN.get().unwrap().lock().unwrap().execute_batch("CREATE TABLE listening_history(id INTEGER PRIMARY KEY, url TEXT, title TEXT, artist TEXT, played_at TEXT, secs INTEGER); CREATE TABLE track_stats(url TEXT PRIMARY KEY, title TEXT, artist TEXT, play_count INTEGER, total_secs INTEGER, first_seen TEXT, last_played TEXT);").unwrap();
        record_play_event("song", "Title", "Artist", 0).unwrap();
        record_play_event("song", "", "", 15).unwrap();
        record_play_event("song", "", "", 3).unwrap();
        let stats = get_listening_stats().unwrap();
        assert_eq!(stats[0].play_count, 1);
        assert_eq!(stats[0].total_secs, 18);
        assert_eq!(stats[0].title, "Title");
        record_play_event("song", "Title", "Artist", 0).unwrap();
        assert_eq!(get_listening_stats().unwrap()[0].play_count, 2);
        *DB_CONN.get().unwrap().lock().unwrap() = Connection::open_in_memory().unwrap();
...
pub fn record_play_event(url: &str, title: &str, artist: &str, secs: i64) -> Result<(), String> {
    ...
    with_db_mut(|conn| {
        let tx = conn.transaction()?;
        if secs > 0 {
            // Listening ticks update elapsed time, not the number of plays.
            tx.execute("UPDATE track_stats SET total_secs = total_secs + ?2 WHERE url = ?1", params![url, secs])?;
            tx.execute("UPDATE listening_history SET secs = secs + ?2 WHERE id = (SELECT id FROM listening_history WHERE url = ?1 ORDER BY id DESC LIMIT 1)", params![url, secs])?;
            tx.commit()?;
            return Ok(());
        }
        tx.execute(
