...
fn get_cache_directories(app: &AppHandle) -> Vec<PathBuf> {
    ...
    let temp_phoebeats = std::env::temp_dir().join("phoebeats");
    if !dirs.contains(&temp_phoebeats) {
        dirs.push(temp_phoebeats);
    }

    #[cfg(target_os = "linux")]
    {
        if let Ok(home) = std::env::var("HOME") {
            let user_cache = PathBuf::from(home).join(".cache").join("phoebeats");
    ...
    #[cfg(target_os = "windows")]
    {
        if let Ok(local_app_data) = std::env::var("LOCALAPPDATA") {
            let win_cache = PathBuf::from(local_app_data).join("phoebeats").join("cache");
...
mod tests {
    ...
    fn test_scan_and_prune_logic() {
        let temp_dir = std::env::temp_dir().join("phoebeats_test_cache_prune");
        ...
        let f_db = temp_dir.join("phoebeats.db");
        ...
        // Only the 3 .tmp files should be scanned; phoebeats.db is protected
