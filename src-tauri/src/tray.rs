...
const TRAY_ID: &str = "phoebeats-tray";
...
fn build_tray(app: &AppHandle) -> Result<(), String> {
    ...
    let show_i = MenuItem::with_id(app, "show", "Show / Hide Phoebeats", true, None::<&str>)
        .map_err(|e| e.to_string())?;
    let sep2 = PredefinedMenuItem::separator(app).map_err(|e| e.to_string())?;
    let quit_i = MenuItem::with_id(app, "quit", "Quit Phoebeats", true, None::<&str>)
        .map_err(|e| e.to_string())?;
    ...
    if let Some(tray) = app.tray_by_id(TRAY_ID) {
        let tooltip = match (title, artist) {
            (Some(t), Some(a)) if !t.is_empty() && !a.is_empty() => {
                ...
                format!("Phoebeats: {status} {a} - {t}")
            }
            (Some(t), _) if !t.is_empty() => {
                let status = if is_playing { "▶" } else { "⏸" };
                format!("Phoebeats: {status} {t}")
            }
            _ => "Phoebeats Music Player".to_string(),
        };
