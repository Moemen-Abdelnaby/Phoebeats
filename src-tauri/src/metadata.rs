...
type MetadataCache = std::collections::HashMap<std::path::PathBuf, (u64, std::time::SystemTime, NativeTrackMeta)>;
static METADATA_CACHE: std::sync::OnceLock<std::sync::Mutex<MetadataCache>> = std::sync::OnceLock::new();

pub fn cached_track_metadata(path: &Path) -> Option<NativeTrackMeta> {
    let stat = std::fs::metadata(path).ok()?;
    let modified = stat.modified().ok()?;
    if let Ok(cache) = METADATA_CACHE.get_or_init(Default::default).lock() {
        if let Some((size, stamp, value)) = cache.get(path) {
            if *size == stat.len() && *stamp == modified { return Some(value.clone()); }
        }
    }
    let fingerprint = format!("v1:{}:{:?}", stat.len(), modified);
    let json = crate::db::load_local_metadata(&path.to_string_lossy(), &fingerprint)?;
    let value: NativeTrackMeta = serde_json::from_str(&json).ok()?;
    if let Ok(mut cache) = METADATA_CACHE.get_or_init(Default::default).lock() {
        if cache.len() >= 10000 { cache.clear(); }
        cache.insert(path.to_path_buf(), (stat.len(), modified, value.clone()));
    }
    Some(value)
}
...
pub fn probe_track_metadata(path: &Path) -> Option<NativeTrackMeta> {
    if let Some(cached) = cached_track_metadata(path) { return Some(cached); }
    ...
    if let Ok(modified) = metadata.modified() {
        if let Ok(json) = serde_json::to_string(&result) {
            crate::db::save_local_metadata(&result.path, &format!("v1:{}:{:?}", size_bytes, modified), &json);
        }
        if let Ok(mut cache) = METADATA_CACHE.get_or_init(Default::default).lock() {
            if cache.len() >= 10000 { cache.clear(); }
            cache.insert(path.to_path_buf(), (size_bytes, modified, result.clone()));
        }
    }
    Some(result)
}
